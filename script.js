  ...
  const renderTodos = () => {
    ...
    todos.forEach((todo, index) => {
      ...
      div.innerHTML = `
        ...
        <button class="todo-delete" data-index="${index}">ðŸ—‘ï¸</button>
      `;
  ...
  const renderHabits = () => {
    ...
    habits.forEach((habit, index) => {
      ...
      div.innerHTML = `
        ...
        <div class="habit-streak">ðŸ”¥ ${habit.streak} days</div>
      `;
...
if (timeDisplay) {
  ...
  const updateDisplay = () => {
    ...
    document.title = isRunning ? `${timeDisplay.textContent} - Focus` : 'Donezo';
  };
  ...
  const renderPlans = () => {
    ...
    plans.forEach((plan, index) => {
      ...
      div.innerHTML = `
        ...
        <button class="btn btn-sm btn-secondary plan-delete" data-index="${index}">ðŸ—‘ï¸</button>
      `;
...
if (gradeInputsContainer) {
  document.getElementById('add-grade-btn').addEventListener('click', () => {
    ...
    row.innerHTML = `
      ...
      <button type="button" class="btn btn-danger btn-sm remove-grade-btn">âœ–</button>
    `;
  ...
  document.getElementById('calc-grade-btn').addEventListener('click', () => {
    ...
    resultBox.innerHTML = `<div>Your Average: <strong style="color: var(--primary); font-size: 2rem;">${avg.toFixed(2)}</strong></div>
                           <div>${avg >= 75 ? 'ðŸŽ‰ Passed!' : 'ðŸ˜¢ Failed.'}</div>`;
  });
}
...
if (pickerBtn) {
  ...
  pickerBtn.addEventListener('click', () => {
    ...
    resultBox.innerHTML = `<div style="font-size: 2rem; color: var(--primary); margin-bottom: 10px;">ðŸ† ${lastPickerWinner}</div>
